    ...
    def test_mitigated_cost_deppolarizing(self):
        for p in self.p:
            with self.subTest(p=p):
                ...
                predicted_mitigated_cost = self.obj.mitigated_cost(self.input,p=p)
                self.assertTrue(numpy.isclose(mitigated_cost, predicted_mitigated_cost))
    ...
    def test_mitigated_variance_dephasing(self):
        for p in self.p:
            with self.subTest(p=p):
                ...
                self.assertTrue(numpy.isclose(var_predicted, var_estimated))
