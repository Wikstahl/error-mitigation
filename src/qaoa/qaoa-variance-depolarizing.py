...
for idx in range(30):
    ...
    params = pickle.load(
        open(path+"qaoa_parameters_minimize_depolarizing", "rb"))
    # Load QAOA with VD parameters
    params_with_vd = pickle.load(
        open(path+"qaoa_parameters_minimize_depolarizing_with_vd", "rb"))
    # Create object
    obj = MyClass(graph)
    ...
    # Save results
    filename = path + "qaoa_variance_depolarizing"
    with open(filename, 'wb') as f:
        pickle.dump(data, f)

    filename = path + "qaoa_variance_depolarizing_with_vd"
    with open(filename, 'wb') as f:
